...
summarize_matrix <- function(x, na.rm=FALSE) {
    ...
    return(data.frame(mean=apply(x,1,mean),stdev=apply(x,1,sd),median=apply(x,1,median),min=apply(x,1,min),max=apply(x,1,max),num_lt_0=apply(x,1,function(i) sum(less_than_zero(i))),num_btw_1_and_5=apply(x,1,function(i) sum(is_between(i,1,5))),num_na=apply(x,1, function(i) sum(is.na(i)))))
}
